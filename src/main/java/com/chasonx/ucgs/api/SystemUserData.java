 ...
 **    Type    Name : SystemUserData 							     	
 **    Create  Time : 2018年4月10日 下午4:12:26								
 ** 																
 ...
 * @create  2018年4月10日下午4:12:26
 * @version 1.0 
 * @desc    
 */
public class SystemUserData extends Controller {

	public void list(){
		Ret ret = new Ret();
		String jsonpName = getPara("jsonpFunc");
		ret.setData(AdminUserDao.getAdminUserList());
		if(ret.getData() != null){
			ret.setResult(1);
		}
		if(StringUtils.hasText(jsonpName)){
			renderJavascript(jsonpName + "("+ JsonKit.toJson(ret) +")");
		}else{
			renderJson(ret);
		}
	}
