 ...
 **    Type    Name : IWorkflowDao 							     	
 **    Create  Time : 2018年5月4日 下午3:07:09								
 ** 																
...
import com.zens.xworkflow.services.IWorkflowBaseDao;

/**
 * @author  Chasonx
 * @email   xzc@zensvision.com
 * @create  2018年5月4日下午3:07:09
 * @version 1.0 
 * @desc    
 */
public class IWorkflowDao extends IWorkflowBaseDao {
	

	static {
		TConfig config = CacheKit.get(Constant.CACHE_DEF_NAME, Constant.Config.WorkFlowServer.toString());
		if(null != config){
			String WORKFLOW_URL = config.getStr("remotehost") + config.getStr("remotedir");
			setUrl(WORKFLOW_URL);
		}
	}
